...
# -*- coding: utf-8 -*-
"""
SOCHIAUTOPARTS Telegram Parser v2.3 (Catch-Up Optimized)
Смарт-догоняние: парсит только новые посты, останавливаясь на границе кеша.
"""
...
CONFIG = {
    ...
    'MAX_PAGES': 30,          # Увеличено с 10
    'MAX_POSTS': 2000,
    'REQUEST_DELAY': (0.3, 0.6),  # Оптимизировано для скорости без бана
    'MAX_RETRIES': 3,
    'RETRY_BACKOFF': 0.3,
    'TIMEOUT': 30,
    'USER_AGENTS': [
...
def parse_poll(wrap: BeautifulSoup) -> Optional[Dict[str, Any]]:
    ...
    for opt in poll.find_all('div', class_='tgme_widget_message_poll_option'):
        ...
        options.append({'text': opt.get_text(strip=True), 'percent': int(pct.get_text().replace('%','')) if pct else None})
    return {'type': 'poll', 'question': question.get_text(strip=True) if question else '', 'options': options}
...
def parse_document(wrap: BeautifulSoup) -> Optional[Dict[str, Any]]:
    ...
    return {
        'type': 'document', 'directUrl': doc.get('href', ''),
        'filename': (doc.find('div', class_='tgme_widget_message_document_title') or {}).get_text(strip=True) or 'Файл',
        'size': (doc.find('div', class_='tgme_widget_message_document_size') or {}).get_text(strip=True)
    }
...
def parse_posts(soup: BeautifulSoup, base_url: str) -> List[Dict[str, Any]]:
    posts = []
    for msg in soup.find_all('div', class_='tgme_widget_message', attrs={'data-post': True}):
        try:
            ...
            if album: media.extend(album)
            else:
                p = parse_photo(msg); if p: media.append(p)
                v = parse_video(msg); if v: media.append(v)
            vo = parse_voice(msg); if vo: media.append(vo)
            d = parse_document(msg); if d: media.append(d)
            poll = parse_poll(msg); if poll: media.append(poll)
            repost = parse_repost_info(msg) or {}
            pid = meta['numericId']
            posts.append({
                'id': pid, 'numericId': pid,
                'date': meta['date'].isoformat(), 'text': text,
                'title': (text[:50] + '...') if len(text) > 50 else text,
                'media': media, 'hasMedia': len(media) > 0, 'mediaCount': len(media),
                'telegramLink': f"{base_url}/{pid}",
                'views': meta['views'], 'forwards': meta['forwards'],
                'keywords': [], 'hashtags': [],
...
def extract_keywords(text: str) -> List[str]:
    ...
    words = re.findall(r'[а-яёa-z]{3,}', cleaned)
    stop = {'и','в','не','что','он','на','я','с','как','а','то','все','она','так','его','но','да','ты','к','у','уже','вы','за','бы','по','только','её','мне','было','вот','от','the','and','for','are','but','not','you','all','can','had','her','was','one','our','out','have','been','will','your','its','from','they','this','that','with','just','what','when'}
...
def main():
    logger.info("=== SOCHIAUTOPARTS Parser v2.3 (Catch-Up) ===")
    ...
    while page <= CONFIG['MAX_PAGES'] and len(new_posts) < CONFIG['MAX_POSTS']:
        ...
        posts = parse_posts(soup, base_url)
        if not posts:
        ...
        for p in posts:
            ...
            for m in p.get('media', []):
                if m.get('directUrl'):
                    media_map[str(hash(m['directUrl']) & 0xFFFFFFFF)] = m['directUrl']
            page_new.append(p)
...
if __name__ == '__main__':
    try: main()
    except KeyboardInterrupt: logger.info("Aborted")
    except Exception as e: logger.exception(f"Critical: {e}"); exit(1)
